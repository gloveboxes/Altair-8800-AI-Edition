...
static char *cells[MAX_ROWS][MAX_COLUMNS];
...
char **cell_slot(int row, int column)
{
...
int loadf()
{
    ...
    position = 0;
    character = fgetc(fp);
    while (character != EOF && character != CPMEOF)
    {
        if (character != CR && character != LF) {
            if (position < sizeof(ln) - 1)
                ln[position++] = (char)character;
