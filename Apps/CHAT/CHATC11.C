...
/*
 * CHATC11 - dcc C11 CP/M 2.2 chat client for the Altair 8800 emulator.
 *
 * This is the C11/Z80 port of the BDS C CHAT app.  It keeps the OpenAI
 * compatible port protocol, includes its JSON request builder, and sends
 * console output through a fully buffered stdout stream.
 *
 * Build with dccmake:
 *   dccmake
 */

#define CHAT_VERSION "1.6-c11"
...
static uint8_t message_types[MESSAGE_HISTORY_LIMIT];
static uint8_t message_count;
static bool response_truncated;
...
static uint8_t wrap_column;
static bool wrap_soft_break;
static uint8_t wrap_word_length;
...
static int timer_port_for(uint8_t timer, bool high);
static int timer_delay_milliseconds(uint8_t timer, uint16_t milliseconds);
...
static int env_get_value(const char *key, char *val, int maxlen);
static int env_set_value(const char *key, const char *val);

static int json_append_text(char *buf, int *pos, int maxlen, const char *text);
static int json_append_escaped_text(char *buf, int *pos, int maxlen, const char *text);
static int json_generate_chat_request(const char *system_message, const uint8_t *types,
                  char *const *texts, int message_total, char *outbuf, int outsize);

static int chat_initialize(void);
static int chat_load_system_prompt(void);
static int chat_run_session(void);
static int chat_add_message(uint8_t type, const char *text);
...
static int chat_receive_response(char *buffer, int bufsize, bool echo);
static int chat_print_wrapped(const char *text);
...
static int chat_store_numeric_config(const char *value, char *destination, bool allow_decimal);
static int chat_store_max_tokens(const char *val);
static int chat_store_temperature(const char *val);
static int chat_store_model(const char *val);
static int chat_show_help(void);
static int chat_show_commands(void);
static bool chat_matches_command(const char *input, const char *full_command, const char *short_command);
...
static int timer_delay_milliseconds(uint8_t timer, uint16_t milliseconds)
{
    ...
    hi_port = timer_port_for(timer, true);
    lo_port = timer_port_for(timer, false);
    if (hi_port < 0 || lo_port < 0)
        return -1;

    outp((unsigned)hi_port, (milliseconds >> 8) & UINT8_MAX);
    outp((unsigned)lo_port, milliseconds & UINT8_MAX);
...
{
    return (int8_t)inp(ENV_COMMAND_PORT);
}

static int env_send_string(const char *value)
{
    while (*value != '\0') {
...
static int env_send_value(const char *value)
{
...
static int env_read_string(char *destination, int destination_size)
{
    ...
    while (index < destination_size - 1) {
        response_byte = (uint8_t)inp(ENV_RESPONSE_PORT);
        if (response_byte == 0)
            break;
...
static int json_append_text(char *buffer, int *pos, int buffer_size, const char *text)
{
    int write_position = *pos;
...
static int json_append_escaped_text(char *buffer, int *pos, int buffer_size, const char *text)
{
    int write_position = *pos;
...
static int chat_show_help(void)
{
    printf("CHATC11 - OpenAI / compatible chat client\n\n");
    printf("Usage: CHATC11 [-H]\n\n");
...
static int chat_store_max_tokens(const char *value)
{
    return chat_store_numeric_config(value, max_tokens_value, false);
}

static int chat_store_temperature(const char *value)
{
    return chat_store_numeric_config(value, temperature_value, true);
}
...
static int chat_send_request(void)
{
    ...
    trimmed_messages = 0;
    request_length = json_generate_chat_request(system_prompt, message_types, message_texts,
                                                 message_count, request_buffer,
    ...
    response_length = chat_receive_response(response_buffer, ASSISTANT_RESPONSE_LENGTH, true);
    ...
    while (pos < bufsize - 1) {
        ...
        if (status == OPENAI_STATUS_DATA_READY) {
            while (status == OPENAI_STATUS_DATA_READY && pos < bufsize - 1) {
                character_value = (uint8_t)inp(124) & 0x7f;
