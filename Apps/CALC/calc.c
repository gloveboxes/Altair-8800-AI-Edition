...
static void set_integer(struct Value *value,
                        const struct CalcInt1024 *int_value) {
  value->is_double = 0;
...
static void parse_power(struct Parser *parser, struct Value *result) {
  ...
  if (!result->is_double && !exponent.is_double &&
    ...
    count = 1;
    while (count < CALC_INT_WORDS && exponent.int_value.word[count] == 0UL)
      ++count;
    if (count < CALC_INT_WORDS || exponent.int_value.word[0] > 32767UL) {
...
static void parse_multiply(struct Parser *parser, struct Value *result) {
  ...
  while (parser->error == 0) {
    ...
    if (integer_divide) {
      ...
      if (op == '*')
        double_status = dmul(&left_double, &right_double, &double_result);
      else
        double_status = ddiv(&left_double, &right_double, &double_result);
      if (double_status != 0) {
        if (double_status == 2)
          parser_error(parser, "Division by zero");
        else
...
static void parse_add(struct Parser *parser, struct Value *result) {
  ...
  while (parser->error == 0) {
    ...
    if (result->is_double || right.is_double) {
      ...
      value_as_double(&right, &right_double);
      if (op == '+')
        double_status = dadd(&left_double, &right_double, &double_result);
      else
        double_status = dsub(&left_double, &right_double, &double_result);
      if (double_status != 0) {
        parser_error(parser, "Double out of range");
        set_int(result, 0);
        return;
