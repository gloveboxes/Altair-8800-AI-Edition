...
static int normalize_precision(struct CalcDouble *value, int precision)
{
    ...
    if (negative) xnegate(&value->coefficient, &absolute);
    else absolute = value->coefficient;
    ...
    while (!xiszero(&absolute)) {
        ...
        if (!xiszero(&remainder)) break;
        absolute = quotient;
        ++value->exponent;
        --length;
    }
    if (negative) xnegate(&absolute, &value->coefficient);
    else value->coefficient = absolute;

    scientific_exponent = value->exponent + length - 1;
    if (scientific_exponent > DOUBLE_MAX_SCI_EXP) return 1;
    if (scientific_exponent < DOUBLE_MIN_SCI_EXP) dzero(value);
    return 0;
}
...
int dparse(const char *text, struct CalcDouble *result)
{
    ...
    coefficient[index] = '\0';
    if (xparse(coefficient, &result->coefficient)) return 1;
...
int disint(const struct CalcDouble *value)
{
    ...
    if (-value->exponent > DOUBLE_DIGITS) return diszero(value);
                             ...
                             const struct CalcDouble *right)
{
    ...
    if (left_scientific < right_scientific) return -1;
    if (left_scientific > right_scientific) return 1;
...
int dcomp(const struct CalcDouble *left, const struct CalcDouble *right)
{
    ...
    if (disneg(left) != disneg(right)) return disneg(left) ? -1 : 1;
         ...
         struct CalcDouble *result)
{
    ...
    if (diszero(left)) { *result = *right; return 0; }
    if (diszero(right)) { *result = *left; return 0; }
    ...
    if (xmul(&higher->coefficient, &factor, &scaled)) return 1;
    if (xadd(&scaled, &lower->coefficient, &result->coefficient)) return 1;
         ...
         struct CalcDouble *result)
{
    ...
    if (!exponent_value(exponent, &power)) return 3;
    negative = power < 0;
    ...
    for (count = 0; count < power; ++count) {
        if (dmul_precision(&product, base, &next,
                           DOUBLE_POWER_DIGITS)) return 1;
        product = next;
    }
    if (negative) {
        if (diszero(&product)) return 2;
        if (ddiv_precision(&one, &product, result,
                           DOUBLE_POWER_DIGITS)) return 1;
        if (normalize(result)) return 1;
    } else {
        if (normalize(&product)) return 1;
        *result = product;
    }
    return 0;
}
...
void dfmt(const struct CalcDouble *value, char *buffer)
{
    ...
    if (point > 0 && point <= DOUBLE_DIGITS) {
        ...
        *output++ = digits[0];
        if (length > 1) {
            *output++ = '.';
            for (index = 1; index < length; ++index) *output++ = digits[index];
        }
        *output++ = 'e';
