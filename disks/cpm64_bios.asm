...
;    bytes found there in a raw disk or RAM dump are not part of the BIOS code.
;
;----------------------------------------------------------------------
;
...
; Set the following two sizes

memsize	equ	64		;memory size
biosLen	equ	0700h		;BIOS length
...
bootLen	equ	3*128		;CP/M boot loader length

; These equates are automatically changed by memSize and biosLen

biosBas	equ	memSize*1024-biosLen	;base address of the BIOS
...
loadTk0	equ	ccpBase-bootLen		;wboot load address for track 0
loadTk1	equ	loadTk0 + 01000h	;wboot load address for track 1

bias	equ	sysgen+bootLen-ccpBase	;bias in DDT to patch in this BIOS
biosGen	equ	biosBas+bias		;BIOS ends up here in SYSGEN image
...
enwdMsk	equ	01h		;enter new write data bit mask
headMsk	equ	04h		;mask to get head load bit alone
moveMsk	equ	02h		;mask to get head move bit alone
trk0Msk	equ	40h		;mask to get track zero bit alone
dSelect	equ	0ffh		;deselects drive when written to drvSel
selMask	equ	08h		;"not used" bit is zero when drive selected

dcStepI	equ	01h		;drive command - step in
dcStepO	equ	02h		;drive command - step out
dcLoad	equ	04h		;drive command - load head
dcUload	equ	08h		;drive command - unload head
dcWrite	equ	80h		;drive command - start write sequence
...
stack	equ	$		;locat stack grows down from here
...
; form the two digit CPM size in ascii (e.g., "60") for "60K")
	...
	lxi	h,biosMem	;end of BIOS to compute memory size
...
tenDone adi	3ah		;compute final 1's digit in ascii
	mov	h,a		;h = 1's digit in ascii
	nop			;64K wraps the computed page marker to zero
	nop			;keep the assembled layout unchanged
	nop
...
wboot	lxi	sp,0100h	;init stack pointer
	...
	xra	a		;select boot drive (drive 0)
	sta	diskNum
	...
	mvi	c,0		;booting from drive 0
	call	seldsk

	mvi	c,0		;starting on track 0
	call	settrk
...
loadTrk	sta	secNum		;save the sector number to we're on
	...
	cpi	(biosBas shr 8) ;current address >= bios start address?
...
entCpm	lxi	b,defDma	;set the default dma address (080h)
	...
	mov	a,m
	ora	a		;set Z true if cold start
	mvi	m,1		;set the flag true - cold start has been done

	lda	flags		;check the "process command line" flag
	jz	coldSt		;process as cold start
...
seldsk	lxi	h,0		;hl of zero is error flag
	...
	mov	a,c		;a=selected disk number
	sta	diskNum		;save in diskNum
...
;----------------------------------------------------------------------------
read	call	swapStk		;swap to local stack
...
;    if a non-recoverable error has occured.
;----------------------------------------------------------------------------
write	call	swapStk		;swap to local stack
	...
	lda	flags		;are we verifying writes?
	ani	fWrtVfy
	jz	exitDio		;no, exit
...
exitDio	push	psw		;save I/O status flags
	lda	flags		;re-enable interrupts?
	ani	fEnaInt
	jz	noInts		;no
	ei			;else, re-enable

noInts	pop	psw		;restore I/O status flags
...
;--------------------------------------------------------------------------
; swapStk - swap to local stack. When the caller of this routine does a
;   RET, the return jumps to "resStk" below which restores the stack of
;   the BIOS caller.
;--------------------------------------------------------------------------
swapStk	pop	d		;de = return address, return address popped
	...
	lxi	sp,stack	;switch to local stack
...
; validate and move data for sectors in tracks 0 - 5
	...
	mov	b,h		;bc->destination address for the data
	mov	c,l
	lxi	h,altBuf+t0Data	;hl->data starting at byte 3 in altBuf
...
rTrk676	dcx	h		;move back to last byte read (offset 136)
	...
	push	b		;sector number off the stack

	lhld	dmaAddr		;hl->destination address for the data
	mov	b,h		;bc->destination address for the data
	mov	c,l
	lxi	h,altBuf+t6Data	;hl->data starting at byte 7 in altbuf
	call	moveBuf		;move cpm sector to (dmaAddr), return checksum

	lxi	h,altBuf+6	;adding bytes 2,3,5 and 6 to checksum
	mov	b,m		;b = byte 6
...
selDrv	lda	diskNum		;diskNum is the disk that CPM wants to use
	mov	e,a		;e=new disk number

	lda	selNum		;a=disk currently selected on the controller
...
;---------------------------------------------------------------------------
flags	db	10h		;enable interrupts after disk i/o is done
...
biosEnd	equ	$		;actual end of the BIOS
biosMem	equ	($+255) and 0ffffh ;16-bit next page boundary


flagOff	equ	flags-biosBas	;offset to flags byte
biosCnt	equ	$-biosBas	;count of bytes in the BIOS
