...
- Target is fixed: `esp32s3`. Toolchain is ESP-IDF **v6.0.3 or newer** (the
...
- ESP-IDF is **not** activated automatically (see user memory: never source
  ...
  ```bash
  source "$HOME/.espressif/v6.0.3/esp-idf/export.sh" >/dev/null
  ```
