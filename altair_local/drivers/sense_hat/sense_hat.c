 ...
 * (src/Drivers/sensehat-driver/src/sense_hat.c).
 */
...
static unsigned char calibrate[32];
...
{
    char filename[32];
    snprintf(filename, sizeof(filename), "/dev/i2c-%d", i2c_num);

    fds.HTS221 = open(filename, O_RDWR);
    if (fds.HTS221 == -1 || ioctl(fds.HTS221, I2C_SLAVE, HTS221_ADDRESS) < 0)
    {
        fprintf(stderr, "[sense-hat] Failed to acquire bus for HTS221 temperature and humidity sensor\n");
        return 0;
    }
    init_HTS221();

    fds.LPS25H = open(filename, O_RDWR);
    if (fds.LPS25H == -1 || ioctl(fds.LPS25H, I2C_SLAVE, LPS25H_ADDRESS) < 0)
    {
        fprintf(stderr, "[sense-hat] Failed to acquire bus for LPS25H pressure sensor\n");
        return 0;
    }
    init_LPS25H();
